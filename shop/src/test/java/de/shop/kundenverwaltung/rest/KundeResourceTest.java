...
public class KundeResourceTest extends AbstractResourceTest {
	...
	private static final Long KUNDE_ID_VORHANDEN_MIT_BESTELLUNGEN = Long
			.valueOf(101);
	...
	private static final String IMAGE_PATH_UPLOAD = "src/test/resources/rest/"
			+ IMAGE_FILENAME;
	private static final String IMAGE_MIMETYPE = "image/png";
	private static final String IMAGE_PATH_DOWNLOAD = "target/"
			+ IMAGE_FILENAME;
	private static final Long KUNDE_ID_UPLOAD = Long.valueOf(102);

	private static final String IMAGE_INVALID = "image.bmp";
	private static final String IMAGE_INVALID_PATH = "src/test/resources/rest/"
			+ IMAGE_INVALID;
	...
	public void findKundeMitBestellungenById() {
		...
		assertThat(response.getLink(SELF_LINK).getUri().toString()).contains(
				String.valueOf(kundeId));

		final URI bestellungenUri = kunde.getBestellungenUri();
		assertThat(bestellungenUri).isNotNull();

		response = getHttpsClient().target(bestellungenUri).request()
				.accept(APPLICATION_JSON).get();
		assertThat(response.getStatus()).isEqualTo(HTTP_OK);

		// Verweist der Link-Header der ermittelten Bestellungen auf den Kunden?
		final Collection<Bestellung> bestellungen = response
				.readEntity(new GenericType<Collection<Bestellung>>() {
				});

		assertThat(bestellungen).isNotEmpty().doesNotContainNull()
				.doesNotHaveDuplicates();
		for (Bestellung b : bestellungen) {
			assertThat(b.getKundeUri().toString()).endsWith(
					String.valueOf(kundeId));
		}
	...
	public void findKundenByNachnameVorhanden() {
		...
		final Collection<Kunde> kunden = response
				.readEntity(new GenericType<Collection<Kunde>>() {
				});
		assertThat(kunden).isNotEmpty().doesNotContainNull()
				.doesNotHaveDuplicates();
	...
	public void findKundenByNachnameNichtVorhanden() {
		...
		assertThat(response.getStatus()).isEqualTo(HTTP_NOT_FOUND);
		final String fehlermeldung = response.readEntity(String.class);
		assertThat(fehlermeldung).isEqualTo(
				"Kein Kunde gefunden mit Nachname " + nachname);
	...
	public void createKundeInvalid() {
		...
		ResteasyConstraintViolation violation = filter(violations)
				.with("message")
				.equalsTo(
						"Lastname must have at least 2 and may only have up to 32 characters.")
				.get().iterator().next();
		...
		violation = filter(violations).with("message")
				.equalsTo("The email address is not valid.").get().iterator()
				.next();
		assertThat(violation.getValue()).isEqualTo(email);

		/*
		 * violation = filter(violations).with("message")
		 * .equalsTo("Password must have at least 8 characters.") .get()
		 * .iterator() .next(); // @ScriptAssert steht bei der Klasse und nicht
		 * bei einem Attribut: // violation.getValue() ruft toString() auf dem
		 * Objekt der Klasse Kunde auf
		 * assertThat(violation.getValue()).contains(
		 * password).contains(passwordWdh);
		 */

		violation = filter(violations).with("message")
				.equalsTo("ZIP code requires 5 digits.").get().iterator()
				.next();
	...
	public void deleteKunde() {
		...
		assertThat(response.getStatus()).isEqualTo(HTTP_OK);
		response.close();
		LOGGER.finer(KUNDEN_URI + "/" + kundeId + "/loeschen");
		response = getHttpsClient(USERNAME_ADMIN, PASSWORD_ADMIN)
				.target(KUNDEN_URI + "/" + kundeId + "/loeschen").request()
				.accept(APPLICATION_JSON).get();

		// Then
		assertThat(response.getStatus()).isEqualTo(HTTP_OK);
		final String respstring = response.readEntity(String.class);
		assertThat(respstring).contains("\"aktiv\":false,");
		response.close();
	...
	public void uploadDownload() throws IOException {
		...
		Response response = getHttpsClient(USERNAME, PASSWORD)
				...
				.resolveTemplate(KundeResource.KUNDEN_ID_PATH_PARAM, kundeId)
				.request().post(entity(uploadBytes, mimeType));
		...
		final String idStr = location.replace(KUNDEN_URI + '/', "").replace(
				"/file", "");
