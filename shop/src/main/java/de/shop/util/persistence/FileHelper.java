...
public class FileHelper implements Serializable {
	...
	private void postConstruct() {
		...
		Context ctx = null;
		try {
			ctx = new InitialContext(); // InitialContext implementiert nicht
										// das Interface Autoclosable
		...
		path = Paths.get(System.getenv("JBOSS_HOME"), "standalone",
				"deployments", "filesDb.war", appName);
		...
		if (Files.exists(path)) {
			...
			LOGGER.errorf(
					"Kein Verzeichnis %s fuer hochgeladene Dateien vorhanden",
					path);
		}
	}
	...
	public MimeType getMimeType(byte[] bytes) {
		...
		try (final InputStream inputStream = new ByteArrayInputStream(bytes)) {
			final String mimeTypeStr = URLConnection
	...
	public String getFilename(Class<? extends Serializable> clazz, Object id,
			MimeType mimeType) {
		final String filename = clazz.getSimpleName() + "_" + id + "."
	...
	public void store(File file) {
		...
		if (Files.exists(absoluteFilename)) {
			...
			try {
				creationTime = Files
						.getFileAttributeView(absoluteFilename,
								BasicFileAttributeView.class).readAttributes()
						.creationTime().toMillis();
			} catch (IOException e) {
				LOGGER.warnf(e,
						"Fehler beim Lesen des Erzeugungsdatums der Datei %s",
						absoluteFilename);
			}

			// Die Datei wurde beim Hochladen evtl. in einem parallelen Thread
			// angelegt,
			// der evtl. vor dem Abspeichern der Verwaltungsdaten in der DB
			// fertig war.
			// Als Zeitunterschied bzw. Toleranz sollten 1000 Millisekunden
			// ausreichend sein.
			if (creationTime + ZEIT > file.getAktualisiert().getTime()) {
		...
		try (final InputStream inputStream = new ByteArrayInputStream(
			...
			LOGGER.warnf(e, "Fehler beim Speichern der Datei %s",
					absoluteFilename);
		}
	}
}
