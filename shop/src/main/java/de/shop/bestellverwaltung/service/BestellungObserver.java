...
public class BestellungObserver implements Serializable {
	...
	private void postConstruct() {
		...
		LOGGER.infof("Absender fuer Bestellung-Emails: %s <%s>", absenderName,
				absenderMail);
	}

	public void onCreateBestellung(
			@Observes @NeueBestellung final Bestellung bestellung) {
		...
		final Runnable sendMail = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					// Empfaenger setzen
					final String vorname = kunde.getVorname() == null ? ""
							: kunde.getVorname();
					final String empfaengerName = vorname + " "
							+ kunde.getNachname();
					...
					// Subject setzen
					message.setSubject("Neue Bestellung Nr. "
							+ bestellung.getId());

					// Text setzen mit MIME Type "text/plain"
					final StringBuilder sb = new StringBuilder(256);
					sb.append("<h3>Neue Bestellung Nr. <b>"
							+ bestellung.getId() + "</b></h3>" + NEWLINE);
					for (Bestellposition bp : bestellung.getBestellpositionen()) {
						sb.append(bp.getAnzahl() + "\t"
								+ bp.getArtikel().getBezeichnung() + "<br/>"
								+ NEWLINE);
					}
